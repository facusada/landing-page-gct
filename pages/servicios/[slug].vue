    ...
    },
    {
      ...
      innerHTML: JSON.stringify({
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: service.title, item: `${siteConfig.url}/servicios/${slug}` }
        ]
      })
    }
  ]
  ...
  <div v-if="service">
    <section class="bg-core-ink py-16 text-white md:py-20">
      <div class="section-shell">
        <nav class="mb-8 text-sm text-white/60" aria-label="Breadcrumb">
          ...
          <NuxtLink to="/servicios" class="hover:text-white">Servicios</NuxtLink>
          <span class="mx-2">/</span>
        ...
        <div class="flex items-start gap-5">
          ...
          <div>
            <p class="text-sm font-extrabold uppercase tracking-[0.2em] text-core-orange">Servicio SAP</p>
    ...
    <section class="bg-white py-16 md:py-24">
      <div class="section-shell grid gap-16 lg:grid-cols-[1.2fr_.8fr]">
        <div>
          <div v-reveal>
            <h2 class="font-display text-3xl font-extrabold text-core-ink">Descripción</h2>
          ...
          <div v-reveal="{ delay: 150 }" class="mt-12">
            <h3 class="font-display text-2xl font-extrabold text-core-ink">Qué incluye</h3>
            <ul class="mt-6 grid gap-3">
              <li v-for="feature in service.features" :key="feature" class="flex items-start gap-3 text-slate-700">
        ...
        <div>
          <div v-reveal="{ delay: 200 }" class="sticky top-28 rounded-xl border border-core-line bg-core-mist p-8">
            <h3 class="font-display text-xl font-extrabold text-core-ink">Beneficios</h3>
            <ul class="mt-5 grid gap-3">
              <li v-for="benefit in service.benefits" :key="benefit" class="flex items-start gap-3 text-slate-700">
      ...
      <div class="section-shell">
        ...
        <div class="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          >
            ...
            <h3 class="font-display text-lg font-extrabold text-core-ink">{{ other.title }}</h3>
            <p class="mt-2 text-sm leading-6 text-slate-600">{{ other.description }}</p>
          </NuxtLink>
        </div>
      </div>
    </section>
